...
def _generate_code_document(file: [CodeSmell]):
    ...
    with doc:
        ...
        with div(id=file[0].location.path):
            h4(
                a("Home", href=f"{link_to_home}index.html"),
                f" > {file[0].location.path}",
            )
...
def _create_code_page(file, output_path):
    ...
    html_path = Path(path.join(output_path, file[0].location.path)).with_suffix(".html")
                        ...
                        with tbody():
                            for smell in report.code_smells:
                                ...
                                if smell.type == Priority.ERROR:
                                    ...
                                    table_data.add(
                                        image(src="warning.svg", alt="warning")
                                    )
                                else:
                                ...
                                row += td(
                                    a(
                                        f"{smell.location.line}:{smell.location.column}",
